document.addEventListener('DOMContentLoaded', () => {
    ...
    musicControl.addEventListener('click', () => {
        ...
        if (isPlaying) {
            ...
            tl.to(musicControl, { scale: 1, rotation: 0, duration: 0.7, ease: 'expo.out' });
        } else {
            bgMusic.play().catch(e => console.error('Audio play failed:', e));
            musicControl.innerHTML = '<i class="fas fa-pause"></i>';
            tl.to(musicControl, { scale: 1.2, rotation: 360, duration: 0.7, ease: 'expo.out' });
            confetti({
                particleCount: 30,
                spread: 20,
    ...
    launchButton.addEventListener('click', () => {
        ...
        if (isHidden) {
            ...
            .fromTo('.celebration', 
                ...
                { scale: 1.05, boxShadow: '0 20px 40px rgba(255, 153, 204, 0.4)', duration: 1.2, ease: 'expo.out' }, '-=1.0'
            );
        }
    });
